			...
			for (int i = (this.perm.size() - 1); i >= 0; i--) {
				...
				for (Edge e : graph.get(v)) {
					if (!(this.perm.containsKey(e.getDest())) && (this.parent.get(v) != e.getDest())) {
						...
						//System.out.println("Checking Edge: " + e.getDest() + " - " + tempNodeDist);
						if (candidate != null) {
							//System.out.println("In if: " + min + " =? " + tempNodeDist + " = " + this.perm.get(v)+ " + " + e.getWeigth());
						}

						if (candidate == null || tempNodeDist < min || min == -1) {
							//System.out.println("cand: " + e.getDest() + " parent: " + v);
							candidate = e;
							min = tempNodeDist;
